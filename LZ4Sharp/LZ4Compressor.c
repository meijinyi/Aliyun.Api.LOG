...
{
	#if LZ4_ARCH64
		#define CLASSNAME LZ4Compressor64
	#else
		#define CLASSNAME LZ4Compressor32
	#endif

    /// <summary>
    /// Class for compressing a byte array into an LZ4 byte array.
    /// </summary>
	public unsafe class CLASSNAME : ILZ4Compressor
    {
        //**************************************
        // Tuning parameters
        //**************************************
...
#include "LZ4.h"
        ...
        {            
            ...
            fixed (byte* deBruijnBytePos = DeBruijnBytePos)
            {
                ...
                // First Byte
                hashTable[LZ4_HASH_VALUE(ip)] = ip - basePtr;
                ...
                for (; ; )
                {
                    ...
                    } while ((r < ip - MAX_DISTANCE) || (A32(r) != A32(ip)));
                ...
                _next_match:
                    ...
                    *(ushort*)op = (ushort)(ip - r); op += 2;
                    ...
                    {
                        size_t diff = (size_t)(AARCH(r) ^ AARCH(ip));
		...
		if (op + (1 + LASTLITERALS) + (len>>8) >= oend) return 0; 		// Check output limit
                    ...
                    // Fill table
                    hashTable[LZ4_HASH_VALUE(ip-2)] = ip - 2 - basePtr;

                    // Test next position
                    r = basePtr + hashTable[LZ4_HASH_VALUE(ip)];
                    hashTable[LZ4_HASH_VALUE(ip)] = ip - basePtr;
                    if ((r > ip - (MAX_DISTANCE + 1)) && (A32(r) == A32(ip))) { token = op++; *token = 0; goto _next_match; }
...
#endif
                    if ((ip < (matchlimit - 1)) && (A16(r) == A16(ip))) { ip += 2; r += 2; }
                    if ((ip < matchlimit) && (*r == *ip)) ip++;
                _endCount:
                    len = (int)(ip - anchor);
                    ...
                    // Fill table
		            hashTable[LZ4_HASH64K_VALUE(ip-2)] = (ushort)(ip - 2 - basep);

                    // Test next position
                    r = basep + hashTable[LZ4_HASH64K_VALUE(ip)];
                    hashTable[LZ4_HASH64K_VALUE(ip)] = (ushort)(ip - basep);
                    if (A32(r) == A32(ip)) { token = op++; *token = 0; goto _next_match; }
